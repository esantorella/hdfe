...
def print_results(pandas_1, pandas_100, groupby_1, groupby_100):
    print(
        "time to compute group means once with Pandas: {0}".format(
            round(pandas_1, n_decimals)
        )
    )
    print(
        "time to compute group means {0} times with Pandas: {1}".format(
            n_iters, round(pandas_100, n_decimals)
        )
    )
    print(
        "time to compute group means once with Grouped: {0}".format(
            round(groupby_1, n_decimals)
        )
    )
    print(
        "time to compute group means {0} times with Grouped: {1}".format(
            n_iters, round(groupby_100, n_decimals)
        )
    )
...
def make_result_df(df: pd.DataFrame):
    ...
    result_df = pd.DataFrame(
        columns=["Pandas", "Groupby"],
        index=pd.MultiIndex.from_product(
            (["Apply", "Transform"], ["Cython", "Python"])
        ),
        data=np.zeros((4, 2)),
    )
